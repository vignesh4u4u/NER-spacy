...
class PDFTextExtrator:
    ...
    def extract_text(self, file, selected_options):
        ...
        if "names" in selected_options:
            ...
            words_to_remove = ["TENANT","LANDLORD","RESIDENT","Tenant","Landlord","Resident","Landlords","Lessee",
                               "LESSEEE","Lessor"]
            filtered_names = [
            ...
            doc1 = PDFTextExtrator.nlp(formatted_json)
            first_final_name = []
            for ent1 in doc1.ents:
                if ent1.label_ in ['PERSON','ORG']:
                    first_final_name.append(ent1.text)
            all_extracted1 = set(first_final_name)
            final_filter_names = {
                f"name_{idx}": name for idx, name in enumerate(all_extracted1, start=1)
            }
            second_formatted_data = {}
            for key, value in final_filter_names.items():
                second_formatted_data[key] = value
            second_formatted_json = json.dumps(second_formatted_data, indent=2)
            doc2 = PDFTextExtrator.nlp(second_formatted_json)
            second_final_name = []
            for ent2 in doc2.ents:
                if ent2.label_ in ['PERSON','ORG']:
                    second_final_name.append(ent2.text)
            final_extracted = set(second_final_name)
            final_extracted_names = {
                f"name_{idx}": name for idx, name in enumerate(final_extracted, start=1)
            }
            data['names'] = final_extracted_names
            data['name_count'] = len(final_extracted_names)
    ...
    def extract_name_dates(self, question, text):
        ...
        clean_text = " ".join(text_list[0:1300])
    ...
    def extract_address(self,question, text, address_list):
        ...
        clean_text = " ".join(words_list[0:1500])
        QA_input = {"question": question, "context": clean_text}
        res = PDFTextExtrator.nlp1(QA_input)
        address = res["answer"]
